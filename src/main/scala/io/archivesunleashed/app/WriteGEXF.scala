...
object WriteGEXF {
  ...
  def makeFile (rdd: RDD[((String, String, String), Int)], gexfPath: String): Boolean = {
    ...
    val edges = rdd.map(r => "<edge source=\"" + ComputeMD5(r._1._2.getBytes).mkString + "\" target=\"" +
      ComputeMD5(r._1._3.getBytes).mkString + "\" weight=\"" + r._2 +
      "\" type=\"directed\">\n" +
      "<attvalues>\n" +
    ...
    val nodes = rdd.flatMap(r => List(nodeStart +
      ComputeMD5(r._1._2.getBytes).mkString + labelStart +
      r._1._2.escapeInvalidXML() + endAttribute,
      nodeStart +
      ComputeMD5(r._1._3.getBytes).mkString + labelStart +
  ...
  def makeFile(data: Array[Row], gexfPath: String): Boolean = {
    ...
    outFile.write("</nodes>\n<edges>\n")
    data foreach { e =>
      outFile.write("<edge source=\"" + ComputeMD5(e.get(1).asInstanceOf[String].getBytes).mkString + "\" target=\"" +
